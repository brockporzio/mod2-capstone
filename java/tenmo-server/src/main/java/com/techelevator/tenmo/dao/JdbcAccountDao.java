...
public class JdbcAccountDao implements AccountDAO{
    ...
    public boolean validateEnoughMoney(BigDecimal transferredAmount, int fromAccountID) {
        if(retrieveBalanceWithAccountID(fromAccountID).compareTo(transferredAmount) == 1 || retrieveBalanceWithAccountID(fromAccountID).compareTo(transferredAmount) == 0) {
            return true;
        }else{
            return false;
        }
